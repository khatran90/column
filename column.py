...
else: a_exp = 1.0 + (N_ratio - 0.1) * (2.0 - 1.0) / (0.7 - 0.1) # Nội suy mũ α
...
with col_charts:
    ...
    with tab1:
        fig_x = go.Figure()
        fig_x.add_trace(go.Scatter(x=M_curve_x, y=N_curve_x, mode='lines+markers', name='Capacity Mx', line=dict(color='#1B365D', width=3)))
        fig_x.add_trace(go.Scatter(x=[M_Edx], y=[N_Ed], mode='markers', name='Design Point', marker=dict(color='Red' if not is_pass else 'Green', size=12, symbol='cross')))
    ...
    with tab2:
        fig_y = go.Figure()
        fig_y.add_trace(go.Scatter(x=M_curve_y, y=N_curve_y, mode='lines+markers', name='Capacity My', line=dict(color='#008080', width=3)))
        fig_y.add_trace(go.Scatter(x=[M_Edy], y=[N_Ed], mode='markers', name='Design Point', marker=dict(color='Red' if not is_pass else 'Green', size=12, symbol='cross')))
...
with col_summary:
    ...
    st.info(f"👉 $(\\frac{{M_{{dx}}}}{{M_{{rdx}}}})^\\alpha + (\\frac{{M_{{dy}}}}{{M_{{rdy}}}})^\\alpha$ = **{round(biaxial_check, 2)}**")
